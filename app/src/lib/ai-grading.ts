...
- 要求外の追加要素（監査カラム、インデックス、制約の追加など）は絶対に減点しないでください
- 模範解答は最小限の例示であることが多く、それ以上の設計は問題ありません
- 【禁止】「模範解答にない要素がある」「余分な要素がある」という理由での減点は行わないでください
...
- 監査カラム（created_at、created_by、updated_at、updated_by など）が追加されていても、絶対に減点しないでください。これらは実務では標準的なベストプラクティスです
- その他の追加カラムや追加制約（CHECK制約、インデックスなど）が模範解答にない場合でも、絶対に減点しないでください
- 模範解答より多いカラムや制約は「余分」ではなく「丁寧な設計」として加点要素で評価してください
- 【禁止】「模範解答にないカラムが含まれている」「余分なカラムがある」という理由での減点は絶対に行わないでください
